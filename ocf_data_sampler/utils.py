...
class Location(BaseModel):
    ...
    def validate_coordinate_system(cls, v):
        ...
        allowed_coordinate_system = ["osgb", "lon_lat", "geostationary", "idx"]
        if v not in allowed_coordinate_system:
            raise ValueError(f"coordinate_system = {v} is not in {allowed_coordinate_system}")
        return v
