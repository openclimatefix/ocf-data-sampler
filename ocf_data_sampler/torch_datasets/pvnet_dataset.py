...
class PVNetDataset(AbstractPVNetDataset):
    ...
    def validate_sample_request(self, t0: pd.Timestamp, location_id: int) -> None:
        ...
        if self.complete_generation:
            ...
            if not (
                t0 in self.valid_t0_and_location_ids["t0"].values
                and location_id in self.valid_t0_and_location_ids[
                    self.valid_t0_and_location_ids["t0"]==t0]["location_id"].values
            ):
                raise ValueError(
