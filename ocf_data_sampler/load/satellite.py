...
def open_sat_data(zarr_path: str | list[str]) -> xr.DataArray:
    ...
    if data_array.dtype.kind not in "bifc":  # boolean, int, float, complex
    ...
    for coord, expected_kind in coord_dtypes.items():
        ...
        if actual_kind != expected_kind:
            ...
            raise TypeError(
                f"Coordinate {coord} should be {expected_kind}, not {actual_kind}",
            )
...
@contextmanager
def _setup_optimal_environment() -> Iterator[None]:
...
def _parse_zarr_path(path: str) -> dict:
    ...
    if (
        components["sha1"] is not None
        and not components["prefix"].endswith(".icechunk")
    ):
        raise ValueError("Commit syntax (@commit) not supported for non-icechunk stores")

    if (
        components["protocol"] == "gs"
        and components["prefix"] is not None
        and "*" in components["prefix"]
    ):
...
def _open_sat_data_icechunk(
    ...
    if protocol is None:
        ...
        with _setup_optimal_environment():
            ...
            logger.debug(
                f"Accessing Ice Chunk repository: {protocol}://{bucket}/{prefix}",
            )
    ...
    try:
        ...
        logger.error(
            f"Failed to open Ice Chunk repository at "
            f"{protocol or 'local'}://{bucket or ''}/{prefix}",
        )
        raise e

    try:
        session = (
            repo.readonly_session(snapshot_id=sha1)
            if sha1
