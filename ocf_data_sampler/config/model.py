...
class NWP(TimeWindowMixin, DropoutMixin, SpatialWindowMixin, NormalisationConstantsMixin):
    ...
    ensemble_member: int = Field(
        None,
        description="If using an ensemble, number of the member to use",
        )

    means_path: str = Field(
        None,
        description="Zarr path to deterministic data that supplements ensemble data if necessary",
        )
