...
def test_slice_datasets_by_time_sites_static(data_sites, t0_str):
    ...
    assert sliced_datasets_dict["site"].dims == ("time_utc", "site_id")
    # Check that capacity_kwp is a coordinate with only the site_id dimension
    assert "capacity_kwp" in sliced_datasets_dict["site"].coords
    assert sliced_datasets_dict["site"].coords["capacity_kwp"].dims == ("site_id",)
